...
mml_conv:	mv	y,octerve
	...
	mv	(!oct_flg),0ffh
; [EXP-B] orig structure: mml_conv_lp starts with mv a,[x++]
	mv	a,[x++]
	cmp	a,'"'
	jrz	mml_conv_lp
	dec	x
...
; [EXP-A] 7 init instructions removed: belong to caller (main2), not beep_out3 local
loop:		dec	ba		
	jrnz	skip1		
part1_co1:	mv	ba,0		
	mv	(!beep),2h	
...
skip13:		nop			
	...
	nop			
	nop			
	nop				; [EXP-C] +1 nop: orig has 7×0x00 in skip13
